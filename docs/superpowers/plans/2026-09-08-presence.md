...
- Modify: `Sources/idle-probe/main.swift` (substitui o arquivo temporário da Task 1)
- Create: `Makefile`
...
Acrescente a `Tests/PresenceCoreTests/IdleReaderTests.swift`:
...
- Produces:
  ...
  - `@MainActor final class PresenceController: ObservableObject` com `state`, `mode`, `lastIdle`, `autoOff`, `turnOn()`, `turnOff()`, `tick() async`
...
- Produces: `PresenceController.screenLocked()`, `PresenceController.screenUnlocked()`, `final class Preferences` com `autoOff: AutoOffInterval`
...
Run: `swift test`
Expected: PASS, 28 testes (2 de IdleReader, 2 de ActivityDeclarer, 21 de controller, 3 de Preferences — o segundo de IdleReader pode aparecer como skipped).
