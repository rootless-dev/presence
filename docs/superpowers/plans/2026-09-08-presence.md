...
- Produces:
  ...
  - O init recebe `initialMode: ActivityMode` — a máquina do Carlos já provou (sonda da Task 2) que `.declared` não zera o contador, e começar toda sessão em `.declared` gastaria ~90s até escalar. O modo verificado é persistido na Task 7.
